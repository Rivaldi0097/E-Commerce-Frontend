...
function ProductReviews() {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_HOSTNAME}/api/reviews/64dccb9d3ff94327259d9ae6`
      )
      .then((res) => {
        if (res.data.length != 0) {
          ...
          for (
            let i = 0;
            i < Math.ceil(res.data.length / reviewsPerPage);
            i++
          ) {
            numbers.push(i + 1);
          }
  ...
  useEffect(() => {
    console.log(reviews);
    let displayData = reviews?.filter(
      (obj: any) => obj.pageValue === currentPageNumber
    );
      ...
      displayData?.map((obj: any) => {
        return (
          <div className="ReviewBox" key={obj._id}>
            <div>
              <p>
                Username: <span className="Username">{obj.username}</span>
              </p>
              <ProductStarReview numberOfStars={obj.rating} />
            </div>
